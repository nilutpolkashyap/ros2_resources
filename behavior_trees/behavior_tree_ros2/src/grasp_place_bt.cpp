...
class ApproachBin : public BT::SyncActionNode
{
	public:
		...
		{
			std::this_thread::sleep_for(5s);
			std::cout <<"Bin aproached" << std::endl;
...
class AskForHelp : public BT::SyncActionNode
{
	public:
		...
		BT::NodeStatus tick() override
		{
			std::cout << "Asking for help. Waiting for 10 seconds....." << std::endl;
...
int main()
{
	...
	BT::PortsList value_ports = {BT::InputPort<std::vector<int>>("ball_location")};
	factory.registerSimpleCondition("BallClose", ballClose, value_ports);
	...
	std::string pkgpath = ament_index_cpp::get_package_share_directory("behavior_tree_ros2");
  	std::string xml_file = pkgpath + "/behavior_tree_xml/grasp_place_bt.xml";
